...
from command_center.services.procfs.exceptions import \
    DoesNotExist, UnknownProcess, NoParentProcess, NotDirectory, NotFile
from command_center.services.procfs.utils import get_module
...
class File(BaseFile):
    ...
    def __init__(self, filepath=None):
        if not filepath:
            name = self.__module__[len('command_center.services.procfs.proc.'):]
...
class ProcessFile(BaseFile):
    ...
    def __init__(self, id, filepath=None):
        if not filepath:
            name = self.__module__[len('command_center.services.procfs.processes.'):]
...
class ProcDirectory(BaseDirectory):
    ...
    _base_module = 'command_center.services.procfs.proc'
    _skip_path_parts = 2
...
class ProcessDirectory(BaseDirectory):
    ...
    _base_module = 'command_center.services.procfs.processes'
    _skip_path_parts = 3
